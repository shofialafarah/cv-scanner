...
<x-guest-layout>
    <div class="mb-6 md:mb-8 text-center px-2">
        ...
        <h1 class="text-2xl md:text-3xl font-black text-white tracking-tight">Selamat Datang</h1>
        <p class="text-slate-500 text-xs md:text-sm mt-2 max-w-[250px] md:max-w-none mx-auto">Masuk untuk melanjutkan analisis CV Anda.</p>
    </div>

    <x-auth-session-status class="mb-4" :status="session('status')" />

    <form method="POST" action="{{ route('login') }}" class="space-y-4 px-1 md:px-0">
        @csrf
        ...
        <div class="space-y-1.5" x-data="{ show: false }">
            <div class="flex justify-between items-center px-1">
                ...
                @if (Route::has('password.request'))
                    <a class="text-[10px] md:text-xs font-bold text-indigo-400 hover:text-indigo-300 transition-colors"
        ...
        <div class="flex items-center ml-1 py-1">
            ...
            <span
                class="ms-3 text-xs md:text-sm font-bold text-slate-500 hover:text-slate-400 cursor-pointer select-none">{{ __('Ingat saya') }}</span>
        </div>
