...
<x-app-layout>
    ...
    <div class="py-8 md:py-12 bg-[#0B0F1A] min-h-screen">
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- Card Utama --}}
            <div class="bg-[#161B2D] border border-slate-800 p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl relative overflow-hidden group">
                {{-- Efek Cahaya --}}
                <div class="absolute -top-24 -right-24 w-48 h-48 bg-indigo-600/10 blur-[80px] rounded-full"></div>

                <div class="relative z-10">
                    <div class="mb-8 text-center">
                        <span class="inline-block px-4 py-1.5 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 text-[9px] md:text-[10px] font-black uppercase tracking-widest mb-4 mx-auto max-w-full truncate">
                            Posisi: {{ $job->title ?? 'Position' }}
                        </span>
                        <h1 class="text-2xl md:text-3xl font-black text-white mb-2">Upload CV</h1>
                        <p class="text-slate-400 text-xs md:text-sm px-2 md:px-4">Pastikan CV dalam format PDF agar AI dapat menganalisis kualifikasimu dengan akurat.</p>
                    </div>
                    ...
                    <form id="uploadForm" action="{{ route('candidates.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div class="space-y-3">
                            <label class="block text-[10px] md:text-sm font-black text-slate-500 uppercase tracking-widest ml-1 flex justify-between">
                                Curriculum Vitae
                                <span class="text-indigo-400 font-mono text-[9px] md:text-[10px]">PDF ONLY</span>
                            </label>
                            ...
                            <div class="relative group cursor-pointer">
                                ...
                                <div id="fileDisplay"
                                    class="bg-slate-900/50 rounded-2xl border-2 border-dashed border-slate-800 p-6 md:p-10 text-center group-hover:border-indigo-500/50 transition-all">
                                    <div id="fileIcon" class="mb-3 flex justify-center">
                                        <svg class="w-8 h-8 md:w-10 md:h-10 text-slate-600 group-hover:text-indigo-400 transition-colors" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                                        </svg>
                                    </div>
                                    <p id="fileName" class="text-xs md:text-sm text-slate-500 font-bold uppercase tracking-tight break-all">
                                        Klik atau tarik file PDF ke sini
                                    </p>
                                </div>
                            </div>
                            <p class="text-[9px] md:text-[10px] text-slate-600 ml-1 italic">* Maksimal ukuran file 2MB</p>
                        </div>

                        <div class="pt-4 text-center">
                            <button type="submit" id="submitBtn"
                                class="group relative w-full bg-indigo-600 text-white font-black py-4 md:py-5 px-6 rounded-2xl transition-all duration-300 hover:bg-indigo-500 shadow-xl shadow-indigo-500/20 overflow-hidden tracking-[0.2em] uppercase text-xs md:text-sm">
